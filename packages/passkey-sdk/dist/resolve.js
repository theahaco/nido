import { Contract, TransactionBuilder, Account, rpc, nativeToScVal, scValToNative, } from "@stellar/stellar-sdk";
 ...
 */
export async function resolveName(rpcUrl, registryContractId, name, networkPassphrase) {
    ...
    const tx = new TransactionBuilder(dummySource, {
        ...
        .addOperation(registry.call("resolve", nativeToScVal(name, { type: "string" })))
        .setTimeout(0)
        .build();
 ...
 */
export async function resolveNameCached(rpcUrl, registryContractId, name, networkPassphrase) {
    ...
    const contractId = await resolveName(rpcUrl, registryContractId, name, networkPassphrase);
    if (contractId) {
        try {
            sessionStorage.setItem(cacheKey, JSON.stringify({ contractId, timestamp: Date.now() }));
        }
        catch {
