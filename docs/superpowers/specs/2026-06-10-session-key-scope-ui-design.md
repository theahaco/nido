...
- Because the policy meters SAC transfers, a limit on the status-message contract scope is vacuous — hence the tip feature: the dApp calls `SAC.transfer(from=smartAccount, to=author, amount)` **directly**, so the auth context is `CallContract(SAC)` and both the scope match and the limit metering apply.
