...
| 3 | Passkeys & On-Chain WebAuthn (deep dive) | ◻️ Planned | The WebAuthn assertion byte-by-byte: `authenticatorData` flags, `clientDataJSON`, the `sha256(authData ‖ sha256(clientData))` digest, secp256r1, and why on-chain verification is trustless. |
